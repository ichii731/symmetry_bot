...
// Get & Post Process
$params = array('q' => '(from:@seikinTV) until:' . $today->format('Y-m-d') . ' since:' . $yesterday->format('Y-m-d') . ' filter:images', 'result_type' => 'recent', 'count' => '100');
$tweets = $connection->get('search/tweets', $params)->statuses;
foreach ($tweets as $val1) {
    foreach ($val1->extended_entities->media as $val2) {
        $title = 'Tweet from seikin(@seikinTV) ID: ' . $val1->id;
